...
Place where you can buy vegetables and fruits
...
```bash
conda create python=3.8 -n plac
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///plac` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE plac WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE plac TO ;
```
...
ERROR:  database "plac" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists plac
